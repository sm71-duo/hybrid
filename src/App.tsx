...
const App = () => {
  ...
  const [stream, setStream] = useState<MediaStream | boolean>();
  const [socket] = useState(io('http://localhost:3000')); // replace with your host machine's IP or public url
  ...
  return (
    <RTCView streamURL={typeof stream === 'object' ? stream?.toURL() : ''} style={styles.viewer} />
  );
};
